...
"""
Numba-accelerated 3D fish schooling simulation (Boids + predator + spherical obstacle)
Ported from https://github.com/vetemaa/fish-simulation (JS / Three.js).
"""
...
def make_default_params(boid_count=400, pred_count=8):
    ...
    p = np.zeros(44, dtype=np.float64)
    p[P_BOUND] = 40.0
    p[P_PLAY_SPEED] = 1.0
...
def _obstacle_avoid(px, py, pz, cx, cy, cz, sphere_r, avoid_r):
    # Simplified: treat obstacle as a sphere, return repulsion when within (sphere_r + avoid_r)
    dx = px - cx
    dy = py - cy
    dz = pz - cz
    d = _norm3(dx, dy, dz)
    ...
    # same style as other rules: (1 - d/influence) magnitude in [0,1]
    mag = 1.0 - d / influence
    ux = dx / d
    uy = dy / d
    uz = dz / d
    ...
    for i in range(boid_count):
        ...
        if en_obs_avoid:
            sx, sy, sz = _obstacle_avoid(boid_pos[i, 0], boid_pos[i, 1], boid_pos[i, 2],
                                         obs_cx, obs_cy, obs_cz, obs_radius, obs_avoid_r)
            ax += sx * obs_avoid_s
            ay += sy * obs_avoid_s
            az += sz * obs_avoid_s

            sx, sy, sz = _obstacle_avoid(boid_pos[i, 0], boid_pos[i, 1], boid_pos[i, 2],
                                         obs2_cx, obs2_cy, obs2_cz, obs_radius, obs_avoid_r)
            ax += sx * obs_avoid_s
            ay += sy * obs_avoid_s
            az += sz * obs_avoid_s

            sx, sy, sz = _obstacle_avoid(boid_pos[i, 0], boid_pos[i, 1], boid_pos[i, 2],
                                         obs3_cx, obs3_cy, obs3_cz, obs_radius, obs_avoid_r)
            ax += sx * obs_avoid_s
            ay += sy * obs_avoid_s
            az += sz * obs_avoid_s
...
# Python side: init + UI
# -----------------------------
def _init_agents(total_boids, total_preds, bound_size, seed=0.1):
...
def run():
    ...
    plt.subplots_adjust(left=0.08, right=0.98, bottom=0.25, top=0.98)
    ...
    labels = ["sep", "ali", "coh", "bnd", "rnd", "pred", "obs"]
    ...
    for s in (s_sep_r, s_ali_r, s_coh_r, s_max, s_play, s_obs_r, s_obs_avoid, s_pred, s_boids):
    ...
    def _update(_frame):
        ...
        return (boid_scatter, pred_scatter, *sphere_lines)
